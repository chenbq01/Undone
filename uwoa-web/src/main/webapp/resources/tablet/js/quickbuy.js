function quickbuy(url, orderno, dishid, dishname, unit, price) {
	jQuery.ajax({
		...
		success : function(data) {
			//var c = $("#" + dishid).html();
			var c = $("span[id='"+dishid+"']").html();
			if (c != "?") {
				if (data == "1") {
					//$("#" + dishid).html(parseInt(c) + 1);
					$("span[id='"+dishid+"']").html(parseInt(c) + 1);
				}
				if (data == "0") {
					$("span[id='"+dishid+"']").html("?");
				}
				...
				success : function(data) {
					...
					*/
					if(dishinfo.pic){
						...
						$("#food_img").attr("src","/uwoa/resources/tablet/images/nopic.png");
					}
					...
					var commentlist_html = "<li data-role=\"list-divider\" role=\"heading\" class=\"ui-li ui-li-divider ui-bar-a ui-li-has-count ui-first-child\">食客的评论 <span class=\"ui-li-count ui-btn-up-a ui-btn-corner-all\">"
							+ data.commentlist.length + "</span></li>";
