...
<body>
	...
	<div id="toolbar_table">
		<div id="but_audit" class="div_but" style="width:80px">审核通过</div>
	...
	<script>
		...
		function search(page) {
			...
			jQuery
					.ajax({
						...
						url : '/uwoa/food/commentManage/query',
						data : jsonuserinfo,
						dataType : 'json',
								...
								$
										.each(
												...
												function(i, item) {
													...
													newData += "<td id=\"l\">" + item.comm_content + "</td>";
													newData += "<td id=\"c\">" + item.comm_time + "</td>";
													newData += "</tr>";
												});
			...
			confirmDialog("删除操作不可恢复，您确认要删除吗？", function(ok) {
				if (ok == true) {
					jQuery.ajax({
						type : 'POST',
						contentType : 'application/json',
						url : '/uwoa/food/commentManage/delete',
						data : jsonuserinfo,
						dataType : 'json',
						success : function(data) {
							if (data && data.success == "true") {
		...
		//审核
		$("#but_audit").button().click(function() {
			var o = getChecked("checkbox");
			if (o.count < 1) {
				alertDialog("请选择至少1条记录进行审核！");
				return false;
			}
			var jsonuserinfo = $.toJSON({
				"ids" : o.ids
			...
			confirmDialog("审核操作不可恢复，您确认要审核吗？", function(ok) {
				if (ok == true) {
					jQuery.ajax({
						type : 'POST',
						contentType : 'application/json',
						url : '/uwoa/food/commentManage/audit',
						data : jsonuserinfo,
						dataType : 'json',
						success : function(data) {
							if (data && data.success == "true") {
