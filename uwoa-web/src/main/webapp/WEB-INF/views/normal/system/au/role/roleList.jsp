...
<%@ page import="cn.com.uwoa.system.au.resource.ResourceHelper" %>
<body>
	...
	<div id="edit_div" title="编辑">
		<form id="edit_form" name="edit_form">
			<table id="main_table">
				<tr>
					<td class="label"><font color="#FF0000">*</font> 角色名称：</td>
					<td class="input"><input type="text" name="name" id="name" validate="notNull" inputName="角色名称" /></td>
		...
		<div id="data_table" class="ui-widget">
			<table class="ui-widget ui-widget-content2">
				<thead>
					<tr class="ui-widget-header ">
						<th width="95%"><font color="#FF0000">*</font> 资源</th>
						<th width="5%"></th>
					</tr>
				</thead>
				<tbody id="sub_tbody">
		...
		$("#but_edit").button().click(
				function() {
					...
					jQuery.ajax({
						type : 'POST',
						contentType : 'application/json',
						url : '/uwoa/system/au/role/queryWithSub',
						data : jsonuserinfo,
						dataType : 'json',
						success : function(data) {
							if (data && data.success == "true") {
								...
								$.each(data.data, function(i, item) {
									...
									$.each(item.sub.data, function(j, subItem) {
										...
										newData += '<td id=\"l\" name=\"d1\"><select name=\"rsid\" id=\"rsid_'+$('#maxRow').val()+'\" validate=\"notNull\" inputName=\"子表-资源\" style=\"width:100%\"><%=ResourceHelper.optionHtml("SELECT").replaceAll("\"", "\\\\\"") %></select></td>';
										...
										$('#edit_div #sub_tbody #row_'+(j+1)+" select[name=rsid]").val(subItem.rsid);
									});
			...
			confirmDialog("删除操作不可恢复，您确认要删除吗？", function(ok) {
				if (ok == true) {
					jQuery.ajax({
						type : 'POST',
						contentType : 'application/json',
						url : '/uwoa/system/au/role/delete',
						data : jsonuserinfo,
						dataType : 'json',
						success : function(data) {
							if (data && data.success == "true") {
						...
						success : function(data) {
							if (data && data.success == "true") {
								...
								$.each(data.data, function(i, item) {
									...
									$.each(item.sub.data, function(j, subItem) {
										newData += "<tr id=\"row_"+(j+1)+"\">";
										newData += "<td id=\"l\">"+subItem.rsid_name+"("+subItem.url+")"+"</td>";
										newData += "</tr>";
									});
						...
						"保存" : function() {
							...
							for(var i=1;i<=rowCount;i++){
								if($("#edit_div #sub_tbody #row_"+i).val()!=undefined){
									var row={};
									var vrow={};
									row["rsid"]=$("#edit_div #sub_tbody #row_"+i+" select[name=rsid]").val();
