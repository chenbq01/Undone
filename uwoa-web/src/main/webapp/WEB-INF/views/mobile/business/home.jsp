	...
	<div data-role="page" data-fullscreen="true">
			...
			<ul data-role="listview" style="margin-top: -10px;" data-inset="true"
				...
				<li><a href="#popupOrgOrderNo" data-rel="popup"
					...
					<div data-role="popup" id="popupMenu" data-theme="a">
						<div data-role="popup" id="popupOrgOrderNo" class="ui-corner-all"
							data-theme="a">
				...
				<c:if test="${sessionScope._profile['open_vip'] == '1'}">
					<li><a href="#popupMobileNo" data-rel="popup"
						data-transition="pop"> <img width="60px" height="60px"
							src="<c:url value="/resources/mobile/images/icon/acdc.png"/>">
							<h2>爱吃的菜</h2>
							<p>验证手机号看看自己在这里最爱吃的菜</p></a>
						<div data-role="popup" id="popupMenu" data-theme="a">
							<div data-role="popup" id="popupMobileNo" class="ui-corner-all"
								data-theme="a">
								<div style="padding: 10px 20px;">
									<h3>请填写并验证手机号</h3>
									<input name="mobileno" value="" placeholder="手机号"
										data-theme="a" type="text"> <a
										id="a_GetMobileRandomCode" href="#" data-role="button"
										data-corners="false" data-theme="b"
										onclick="javascript:GetMobileRandomCode();">获取验证码</a> <input
										name="randomno" value="" placeholder="手机验证码" data-theme="a"
										type="text"> <a id="a_ToFavourite" href="#"
										data-role="button" data-inline="true" data-theme="b"
										data-icon="check" data-corners="false"
										onclick="javascript:ToFavourite();">确定</a><a href="#"
										data-role="button" data-inline="true" data-rel="back"
										data-theme="a" data-corners="false">取消</a>
								</div>
							</div>
		...
		<script type="text/javascript">
			...
			function ToFavourite() {
				var mobileno = $("input[name='mobileno']");
				var randomno = $("input[name='randomno']");
				var value = mobileno.val();
				...
				jQuery.ajax({
						...
						success : function(data) {
							...
							if (data.length == 0) {
								...
								{
									goUrl = url.substring(0,url.indexOf(";jsessionid")) + "/" + value + url.substring(url.indexOf(";jsessionid"))
								}
								else
								{
									goUrl = url + "/" + value ;
	...
	<div data-role="page" id="buffet">
		...
		<ul data-role="listview" style="margin-top: -10px;" data-inset="true"
			...
			<c:forEach var="buffet" items="${buffetlist}">
				<li data-icon="check"><a
					href="<c:url value="/dishes/buffet/${buffet['id']}"/>"> <img
						width="60px" height="60px" src="${buffet['pic']}">
						<h2>${buffet['buffet_name']} ${buffet['price']}</h2>
						<p>${buffet['memo']}</p></a></li>
			</c:forEach>
		</ul>
	</div>
	<script type="text/javascript">
	<c:if test="${buffetlist.size()>0&&sessionScope._buffetid==null}">
	//$.mobile.navigate('#buffet');
	//$.mobile.changePage('#buffet');
	</c:if>
	</script>
</body>