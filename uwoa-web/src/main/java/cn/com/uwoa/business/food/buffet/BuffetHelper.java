package cn.com.uwoa.business.food.buffet;
...
import cn.com.uwoa.business.food.buffet.web.controller.BuffetController;
...
public class BuffetHelper {
	...
	public static String optionHtml(String type){
		//获得数据字典控制器
		BuffetController c=SpringContextHolder.getBean("buffetController");
		...
		for(int i=0;i<queryData.size();i++){
			...
			returnString.append("<option value=\""+row.get("id")+"\" price=\""+row.get("price")+"\">"+row.get("buffet_name")+"</option>");
		}
	...
	public static String checkboxListHtml(){
		...
		for(int i=1;i<=lst.size();i++){
			...
			listRowHtml+="<td width=\"3%\"><input type=\"checkbox\"  name=\"buffet_id_"+id+"\" style=\"width:20px\" value=\""+id+"\"/></td><td width=\"30%\">"+((Map)lst.get(i-1)).get("type_name")+"</td>";
			if(i%3==0){
