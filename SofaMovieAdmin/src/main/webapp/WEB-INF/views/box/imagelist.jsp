...
<body>
	...
	<div id="wrapper">
		...
		<div id="content">
			...
			<div class="container">
				<a href="<c:url value="/box/manage"/>?regionid=${regionid}"
				...
				<div id="uploadform" style="display: none; cursor: default">
					...
					<div class="content clearfix">
						...
						<form class="form-horizontal" enctype="multipart/form-data"
							action="<c:url value="/boximageinfo/uploadfile" />" method="post">
							<input type="hidden" name="boxid" value="${boxid}" /><input
								type="hidden" name="boxname" value="${boxname}" /><input
	...
	<script>
		...
		function removeImage(id) {
			var boxname = encodeURI(encodeURI('${boxname}'));
			window.location.href = "<c:url value="/boximageinfo/removeImage"/>?boxid=${boxid}&boxname="
					+ boxname + "&regionid=${regionid}&id=" + id;
		}
		function loadImageList() {
			//$(".plan-container .plan-title").html("");
			$
					.get(
							"<c:url value="/boximageinfo/list"/>?boxid=${boxid}&_json=true",
							function(data) {
								if (data.length > 0) {
									var j = 0;
									$(".plan-container")
